...
public:
	...
	int mWallDir = 0; // 1 - �ciana po prawej ; -1 - �ciana po lewej
	...
	int mAttackDir = 0; // 0 - boki, 1 - g�ra, 2 - d�
	int mAttackDamage = 1;
	...
	bool mLoopAnim = true; // czy animacja si� powtarza?
	...
	// Sprawdzamy, jakich wrog�w trafi� gracz aby nie zada� im obra�e� wi�cej ni� raz
...
private:
	...
	void handleSacrifice() {
		// Mechanika ta sprawia, �e bro� specjaln�, kt�r� trzyma gracz zostaje zamieniona na jego punkty zdrowia
	...
	void handleWallMovement(float deltaTime) {
		...
		if (mTouchingWall && mStamina > 0) {
			...
			// Gdy stoimy przy �cianie, nie chcemy si� do niej przykleja�
	...
	void handleMovement(float deltaTime) {
		...
		float airAccel = 1200.0f; // normalne przy�pieszenie w powietrzu
		float airTurnAccel = 3000.0f; // szybkie hamowanie/nawracanie
		float airDrag = 500.0f; // op�r powietrza
		...
		if (mIsGrounded) {
			...
			if (isSuperSpeed) {
				...
				if ((mVelocity.x > 0 && moveDir > 0) || (mVelocity.x < 0 && moveDir < 0)) {
					drag *= 0.5f; // przytrzymuj�c przycisk wyd�u�asz "�lizg"
				}
	...
	void handleJump(float deltaTime) {
		...
		if (IsKeyPressed(KEY_SPACE) || mJumpBufferCounter > 0) {
			...
			if (mTouchingWall && !mIsGrounded) {
				...
				// �ciana po prawej, skaczemy w lewo -> facingRight = false
				// �ciana po lewej, skaczemy w prawo -> facingRight = true
				mIsFacingRight = (mWallDir == -1);
				
				mIsClimbing = false;
				mJumpCount = 1; // wall jump mo�e resetowa� skok
				jumped = true;
			}
...
public:
	...
	void onCollision(Entity* pOther) override {
		if (pOther->mType == WALL) {
			...
			Rectangle otherRect = pOther->getRect(); // prostok�t �ciany

			// dolna kraw�d� gracza w poprzedniej klatce
			float prevBottom = mPrevPosition.y + mSize.y;

			// lewa i prawa kraw�d� w poprzedniej klatce
			float prevRight = mPrevPosition.x + mSize.x;
			float prevLeft = mPrevPosition.x;

			// podloga
			if (prevBottom <= otherRect.y + 2.0f) { // 2.0f toleracji dla float�w 
				...
				// (WAVEDASH LOGIC) je�li dotkneli�my ziemi podczas dash'a
				if (mIsDashing) {
					mIsDashing = false;
			...
			// sufit
			else if (mPrevPosition.y >= otherRect.y + otherRect.height - 2.0f) { // 2.0f tolerancji dla float�w
			...
			// �ciany
				...
				if (prevRight <= otherRect.x + 5.0f) { // margines b��du
					mPosition.x = otherRect.x - mSize.x;
					mTouchingWall = true;
					mWallDir = 1.0f; // �ciana po prawej
				}
				// czy entity by�o z prawej strony
				else if (prevLeft >= otherRect.x + otherRect.width - 5.0f) { // margines b��du
					mPosition.x = otherRect.x + otherRect.width;
					mTouchingWall = true;
					mWallDir = -1.0f; // �ciana po lewej
				}
	...
	void draw() override {
		...
		if (mStamina < mMaxStamina) {
			...
			Color color = ColorAlpha(RED,percent); // Czerwony zanika gdy ko�c�wka si�
		...
		if (mIsAttacking) {
			...
			if (mAttackDir == 1) {
				offset = { 0.0f, -10.0f };
				rotation = mIsFacingRight ? -90.0f : 90.0f; // g�ra
			}
		...
		Rectangle dest = {
			...
			16.0f, // rozmiar wy�wietlania
			16.0f,
		};

		// MRUGANIE PO OTRZYMANIU OBRA�E�
		...
		if (mFrameTimer >= mFrameSpeed) {
			...
			if (mCurrentFrameRelative >= mLengthFrames) {
				if (mLoopAnim) {
					...
					// LOGIKA WYJ�CIA Z TURN
					// Gdy animacja obrotu si� sko�czy�a, decydujemy co dalej
					if (mCurrentState == TURN) {
						// Je�li nadal biegnie lub trzyma klawisz -> WALK, w przeciwnym razie IDLE
	...
	void handleGhosts(float deltaTime) {
		...
		if (mIsDashing || isSuperSpeed && !isInRecoil) {
			...
			if (mGhostSpawnTimer <= 0) {
				...
				newGhost.alpha = 0.5f; // startowa prze�roczysto��

				mGhosts.push_back(newGhost);
			}
		}
		else {
			mGhostSpawnTimer = 0; // reset, aby po wci�ni�ciu dasha duch pojawi� si� od razu
		}
